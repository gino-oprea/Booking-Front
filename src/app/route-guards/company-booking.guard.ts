...
@Injectable()
export class CompanyBookingGuard implements CanActivate
{
  constructor(private companyService: CompanySearchService, private router: Router) { }

  public canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<boolean> | Promise<boolean> | boolean 
  {
    ////se verifica daca compania din ruta chiar exista in baza de date
    ...
    return this.companyService.getCompanies(flt).pipe(map(result =>
    {
      ...
      if (gro.objList.length > 0)
      {
        ...
        if (company.id == companyId && companyName == company.name.replace(/ /g, ''))
          return true;
        else
